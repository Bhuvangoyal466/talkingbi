...
class ChartTypeSelector:
    ...
    def select(self, intent: str, extracted_data: dict) -> dict:
        # Safety-net: honour any [chart_type:xxx] prefix that arrives via the /chat route
        if intent.startswith("[chart_type:"):
            try:
                forced = intent[12:intent.index("]")].strip().lower()
                if forced in SUPPORTED_CHARTS:
                    return {
                        "recommended_chart_type": forced,
                        "justification": f"User explicitly selected: {forced}",
                        "confidence_score": 10,
                    }
            except ValueError:
                pass
